...
# 这里只以dsrc作为实验数据例子
def readtxt(check_ttype, light_node_data):
    if check_ttype != 'dsrc':
        print('error input')
        return [], []
...
def transactions():
    ...
    if checktype == 'dsrc':
        ...
        # 写入区块对应位置
        index = blockchain.dsrc(values['T-Type'], values['Sender'], values['Receiver'], values['M-Type'], values['Msg'],
                                values['Communication object'])
        ...
        if not all(k in check for k in values):
            return 'Missing Data input', 400
        # 写入区块对应位置
        index = blockchain.fees(values['T-Type'], values['Sender'], values['Receiver'], values['M-Type'], values['Msg'],
                                values['Fees'])
        response = 'We will add this transactions to block%s' % index
        return jsonify(response), 201
...
def transaction_check():
    ...
    if values is None:
        return 'None data input', 400
    core_list, neighbour_list = readtxt(checktype, values)
    response = {
        "core_list": core_list,
